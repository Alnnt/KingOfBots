...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.cells = [new Cell(info.r, info.c)];  //存放蛇身，cell[0]为蛇头
        this.next_cell = null;  //下一步的目标位置

        this.speed = 5;       // 蛇每秒移动格子数
        this.direction = -1;  // -1没有指令，0、1、2、3表示上右下左
        this.status = "idle"; // idle - 静止， move - 正在移动， die - 死亡

        this.dr = [-1, 0, 1, 0];  // 各方向行偏移量
        this.dc = [0, 1, 0, -1];  // 各方向列偏移量
        ...
        this.eye_dx = [     // 蛇眼各方向x偏移量
            [-1, 1],
            [1, 1],
            [1, -1],
            [-1, -1]
        ]
        this.eye_dy = [     // 蛇眼各方向y偏移量
            [-1, -1],
            [-1, 1],
            [1, 1],
            [-1, 1]
    ...
    check_tail_increasing() {       // 检测当前回合是否可以增加蛇长度 (是否在蛇长度开局增长阶段）
    ...
    next_step() {   // 将蛇的状态变为走下一步
        ...
        if (!this.gamemap.check_valid(this.next_cell)) {    // 撞障碍物
            this.status = 'die';
        }
    }
